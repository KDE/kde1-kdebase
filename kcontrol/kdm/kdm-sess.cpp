...
void KDMSessionsWidget::setupPage(QWidget *p)
{
      ...
      QGroupBox *group = new QGroupBox( klocale->translate("Commands"), this );
      group->move( 5, sdcombo->y()+sdcombo->height()+10 );
      label = new QLabel(klocale->translate("Shutdown"), group);
      label->move(5, 20);
      label->adjustSize();
      shutdown_lined = new QLineEdit(group);
      shutdown_lined->setGeometry(label->width()+10,20,
                       p->width()-(label->width()+50 ),
                       shutdown_lined->sizeHint().height());

      label = new QLabel(klocale->translate("Restart"), group);
      label->move(5, shutdown_lined->y()+shutdown_lined->height()+10);
      label->adjustSize();

      restart_lined = new QLineEdit(group);
      ...
      group = new QGroupBox( klocale->translate("Session types"), this );
      group->move(5, y+10);
      
      label = new QLabel(klocale->translate("New type"), group);
      label->move(5, 20);
      label->adjustSize();

      session_lined = new QLineEdit(group);
      ...
      QButton *btnadd = new QPushButton( klocale->translate("Add"), group );
