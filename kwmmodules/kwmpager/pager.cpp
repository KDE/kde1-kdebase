...
Pager::Pager(KWMModuleApplication *a)
{
  ...
  connect(kwmmapp, SIGNAL( desktopNumberChange(int)), SLOT(changeNumber(int)));
  connect(kwmmapp, SIGNAL( windowAdd(Window)), SLOT(addWindow(Window)));
  connect(kwmmapp, SIGNAL( windowRemove(Window)), SLOT(removeWindow(Window)));
  connect(kwmmapp, SIGNAL( windowChange(Window)), SLOT(windowChange(Window)));
  connect(kwmmapp, SIGNAL( windowRaise(Window)), SLOT(windowChange(Window)));
  connect(kwmmapp, SIGNAL( windowLower(Window)), SLOT(windowChange(Window)));
  connect(kwmmapp, SIGNAL( windowActivate(Window)), SLOT(windowActivate(Window)));
...
void Pager::resizeEvent ( QResizeEvent * )  
{
  ...
  Desktop *desk;
  int i;
  for (desk = desktops.first(), i=0; desk != 0L; desk = desktops.next(), i++) {
