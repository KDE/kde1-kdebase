...
void Manager::switchDesktop(int new_desktop){
  ...
  sendToModules(module_desktop_change, 0);
}
...
void Manager::logout(){
  ...
  for (c = clients.first(); c; c = clients.next()){
    if (c->Psaveyourself){
      ...
      do {
	XWindowEvent(qt_xdisplay(), c->window, PropertyChangeMask, &ev);
...
QStrList* Manager::getSessionCommands(){
  ...
  for (c = clients.first(); c; c = clients.next()){
    if (!c->command.isEmpty()){
      ...
      QString command = c->command.data();
      if (!c->machine.isEmpty()){
	if (c->machine != machine && c->machine != all){
	  command.prepend(" ");
